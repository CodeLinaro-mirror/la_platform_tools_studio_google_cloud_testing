...
private const val PACKAGE_CLASS_REGEX = "\\tat (?<$FULL_CLASS_NAME>(?<$PACKAGE_NAME>.+)\\.(?<$CLASS_NAME>.+?))\\.(?<method>.+?)"

private val fileAndLineRegex = Regex("$PACKAGE_CLASS_REGEX\\((?<$LOCATION>(?<$FILE_NAME>.+):(?<$LINE_NUMBER>\\d+)\\))")

private val otherMethodRegex = Regex("$PACKAGE_CLASS_REGEX\\((?<$SOURCES>(Native Method|Unknown Source)\\))")

/**
 * Parses exceptions from log returned by Firebase test lab and generates hyperlinks to the corresponding file.
 * This filter is used in all console windows.
 *
 * In log emitted by FTL devices, we see that exception message is formatted as below:
 * 08-07 16:44:16.268: E/TestRunner(9723): 	at com.example.ftlgmdtestproject.ExampleInstrumentedTest2$TestOnly.fail(ExampleInstrumentedTest2.kt:34)
 *
 * A regular Gradle managed device or a connected device would have the following output:
 * 08-07 10:51:07.672  2798  2997 E TestRunner: 	at android.app.Instrumentation$InstrumentationThread.run(Instrumentation.java:2248)
 *
...
class TestLabExceptionFilter(
  ...
  private fun createHyperLinkFromMatch(
    match: MatchResult, offset: Int, range: IntRange, lineNumber: Int, matchFileName: Boolean
  ): Filter.ResultItem? {
  ...
  // Nested classes are formatted as OutClass$NestedClass1$NestedClass2. We format the class name to OutClass.NestedClass1.NestedClass2
  // to match return result from PsiClass.qualifiedName to filter the classes we found in fileNamesCache. After filtering the
  ...
  private fun findFilesForMatch(match: MatchResult, matchFileName: Boolean): List<VirtualFile> {
    val expandedClassName = match.groups[CLASS_NAME]?.value?.split("$")?.filter { it.toIntOrNull() == null } ?: return emptyList()
