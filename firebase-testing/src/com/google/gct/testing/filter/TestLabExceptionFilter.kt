...
/**
 * Parses exceptions from log returned by Firebase test lab and generates hyperlinks to the corresponding file. This filter is used in all
 * console windows.
 *
 * In log emitted by FTL devices, we see that exception message is formatted as below: 08-07 16:44:16.268: E/TestRunner(9723): at
 * com.example.ftlgmdtestproject.ExampleInstrumentedTest2$TestOnly.fail(ExampleInstrumentedTest2.kt:34)
 *
 * A regular Gradle managed device or a connected device would have the following output: 08-07 10:51:07.672 2798 2997 E TestRunner: at
 * android.app.Instrumentation$InstrumentationThread.run(Instrumentation.java:2248)
 *
 * The parenthesis in string "E/TestRunner(9723)" causes current exception filter to fail since the parser only recognizes the first "(" and
 * ignores content in "(Instrumentation.java:2248)".
 */
class TestLabExceptionFilter(private val myProject: Project, private val mySearchScope: GlobalSearchScope) : Filter, DumbAware {
